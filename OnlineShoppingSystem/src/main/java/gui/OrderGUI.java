...
/**
 * Place-order screen, styled the same way as Login.java (same palette,
 * fonts, rounded fields, art panel — all self-contained in this file).
 * Same Order model and OrderDAO.placeOrder(Order) as your teammate's original.
 */
public class OrderGUI extends JFrame {
    ...
    private JLabel productError, quantityError, totalError, banner;
    private JButton orderButton;

    public OrderGUI() {
        setTitle("Place order — Kartly");
        setSize(880, 600);
    ...
    private JPanel buildFormPanel() {
        ...
        col.add(leftLabel("Checkout", FONT_KICK, MUTED));
        col.add(Box.createVerticalStrut(4));
        col.add(leftLabel("Place order", FONT_TITLE, INK));
        ...
        productField = addField(col, "Product name");
        productError = errorLabel();
        col.add(productError);
        ...
        totalField = addField(col, "Total price");
        totalError = errorLabel();
        col.add(totalError);
        col.add(Box.createVerticalStrut(14));

        col.add(fieldLabelRow("Payment status", null));
        col.add(Box.createVerticalStrut(7));
        paymentBox = new JComboBox<>(new String[]{"Paid", "Pending"});
        paymentBox.setFont(FONT_FIELD);
        paymentBox.setBackground(Color.WHITE);
        paymentBox.setForeground(INK);
        ...
        paymentBox.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
        col.add(paymentBox);
        col.add(Box.createVerticalStrut(24));

        orderButton = roundedButton("Place order");
        orderButton.setAlignmentX(Component.LEFT_ALIGNMENT);
        orderButton.setMaximumSize(new Dimension(Integer.MAX_VALUE, 48));
        orderButton.addActionListener(e -> attemptPlaceOrder());
        col.add(orderButton);
    ...
    private JComponent buildArtPanel() {
        JComponent art = new JComponent() {
            protected void paintComponent(Graphics g) {
                ...
                g2.fill(new java.awt.geom.Ellipse2D.Float(cx - 130, cy - 130, 260, 260));

                int x = cx - 70, y = cy - 110;
                g2.setColor(Color.WHITE);
                g2.fill(new RoundRectangle2D.Float(x, y, 140, 180, 14, 14));
                g2.setColor(LINE);
                g2.setStroke(new BasicStroke(2f));
                g2.draw(new RoundRectangle2D.Float(x, y, 140, 180, 14, 14));
                g2.setColor(CORAL);
                for (int i = 0; i < 4; i++) {
                    g2.fillRoundRect(x + 20, y + 30 + i * 34, 100, 12, 6, 6);
                }
                g2.setColor(TEAL);
                g2.fillOval(x + 100, y + 150, 34, 34);
                g2.setColor(Color.WHITE);
                g2.setStroke(new BasicStroke(3f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                g2.drawLine(x + 110, y + 167, x + 116, y + 174);
                g2.drawLine(x + 116, y + 174, x + 128, y + 158);
        ...
        JLabel q1 = new JLabel("One tap,");
        JLabel q2 = new JLabel("on its way.");
    ...
    private void attemptPlaceOrder() {
        ...
        if (productName.isEmpty()) {
            setFieldError(productField, productError, "Enter a product name.");
            ok = false;
        }
        ...
        if (qtyText.isEmpty()) {
            ...
            try {
                ...
                if (quantity <= 0) {
                    setFieldError(quantityField, quantityError, "Quantity must be greater than 0.");
                    ok = false;
                }
        ...
        if (totalText.isEmpty()) {
            setFieldError(totalField, totalError, "Enter the total price.");
            ok = false;
        } else {
            try {
                total = Double.parseDouble(totalText);
                if (total <= 0) {
                    setFieldError(totalField, totalError, "Total price must be greater than 0.");
                    ok = false;
                }
            } catch (NumberFormatException ex) {
                setFieldError(totalField, totalError, "Total price must be a number, e.g. 49.99.");
                ok = false;
            }
        }
        ...
        Order order = new Order(productName, quantity, total, payment);
        OrderDAO dao = new OrderDAO();
        boolean result = dao.placeOrder(order);

        if (result) {
            showBanner("Order placed successfully.", OK);
        } else {
            showBanner("Order failed. Please try again.", ERROR_COLOR);
        }
    }
    ...
    private void clearErrors() {
        productField.setErrorState(false);
        quantityField.setErrorState(false);
        totalField.setErrorState(false);
